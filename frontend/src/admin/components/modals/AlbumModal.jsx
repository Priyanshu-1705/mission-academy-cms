...
export function AlbumModal({ editingAlbum, albumForm, setAlbumForm, handleSaveAlbum, setIsAlbumModalOpen }) {
  const [isDragging, setIsDragging] = useState(false);
  const [fileError, setFileError] = useState("");

  const handleChange = (field, val) => {
    setAlbumForm(prev => ({ ...prev, [field]: val }));
  };
  ...
  return (
    <div className="fixed inset-0 z-[110] bg-black/60 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl max-w-md w-full p-6 space-y-5 shadow-2xl relative max-h-[90vh] overflow-y-auto">
        <button onClick={() => setIsAlbumModalOpen(false)} className="absolute top-4 right-4 text-gray-400 hover:text-gray-600">
          <X className="h-5 w-5" />
        </button>
        <h3 className="text-lg font-bold text-gray-900">{editingAlbum ? "Edit Gallery Album" : "Create Gallery Album"}</h3>
        <div className="space-y-4">
          ...
          <div className="space-y-1">
            ...
            <textarea value={albumForm.description || ""} onChange={e => handleChange("description", e.target.value)} className="w-full border border-gray-200 px-4 py-2 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-school-primary/20" rows={2}></textarea>
          </div>

          {/* Cover Image Dropzone */}
          <div className="space-y-2">
            <label className="text-xs font-bold text-gray-700 uppercase">Cover Image</label>
            <div
            ...
            {albumForm.coverImage && (
              <div className="space-y-1.5">
                <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block">Cover Image Preview</span>
                <div className="relative rounded-xl overflow-hidden border border-gray-200 bg-gray-50 flex items-center justify-center h-40">
                  <img
                    src={
                      albumForm.coverImage instanceof File
                        ? URL.createObjectURL(albumForm.coverImage)
                        : albumForm.coverImage
                    }
                    alt="Preview"
                ...
                <div className="text-emerald-600 text-xs font-bold px-1 flex items-center space-x-1.5 bg-emerald-50 py-1.5 px-2.5 rounded-lg border border-emerald-100">
                  ...
                  <span className="truncate">Cover image loaded successfully</span>
                </div>
              </div>
            )}
          ...
          <div className="space-y-1">
            <label className="text-xs font-bold text-gray-700 uppercase">Event Date (YYYY-MM-DD)</label>
            <input type="date" value={albumForm.date || ""} onChange={e => handleChange("date", e.target.value)} className="w-full border border-gray-200 px-4 py-2 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-school-primary/20" />
          </div>
        </div>
        <div className="flex justify-end space-x-2 pt-4 border-t border-gray-100">
          <button onClick={() => setIsAlbumModalOpen(false)} className="px-4 py-2 border border-gray-200 rounded-xl text-sm font-bold text-gray-500 hover:bg-gray-50">Cancel</button>
          <button onClick={handleSaveAlbum} className="inline-flex items-center space-x-2 bg-school-primary text-white px-4 py-2 rounded-xl text-sm font-bold hover:bg-opacity-90">
